...
test('distributable includes corresponding service source, license, build and replay inputs',()=>{
  ...
  for(const entry of ['dist','src','service/src','service/test','service/build.sh','service/LICENSE','scripts/run-job-c.mjs','scripts/run-job-d.mjs','scripts/run-job-e.mjs','scripts/run-job-f.mjs','scripts/verify-official.mjs','fixtures/golden','fixtures/official','docs'])assert.ok(pkg.files.includes(entry),`missing package input ${entry}`);
